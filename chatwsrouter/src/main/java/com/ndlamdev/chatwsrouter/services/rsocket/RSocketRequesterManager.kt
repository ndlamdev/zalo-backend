...
import com.ndlamdev.chatwsrouter.utils.properties.RSocketClientsProperties
...
class RSocketRequesterManager(
    ...
    private val chatWsServiceRequesterMap: MutableMap<String, RSocketRequester> = mutableMapOf()
    ...
    fun getOrPutChatWsServiceRequester(metadata: RSocketMetadata): RSocketRequester {
        val key = "ChatWsServiceRequest_${metadata.host}:${metadata.port}"
        return chatWsServiceRequesterMap.getOrPut(key) {
            requesterBuilder
                .rsocketConnector { rSocketConnector: RSocketConnector? ->
                    rSocketConnector!!.reconnect(
                        Retry.fixedDelay(2, Duration.ofSeconds(2))
                ...
                .dataMimeType(MimeTypeUtils.parseMimeType(metadata.dataMime))
                .tcp(metadata.host, metadata.port)
        }
    }
}

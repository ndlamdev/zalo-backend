...
interface IGrpcMapper {
    ...
    fun toBoolean(value: BoolValue): Boolean? {
        if (BoolValue.getDefaultInstance() == value) return null
        return value.value
    }

    fun toBoolValue(value: Boolean?): BoolValue? {
        if (value == null) return BoolValue.getDefaultInstance()
